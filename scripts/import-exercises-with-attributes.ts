...
function groupExercisesByOriginalId(rows: ExerciseAttributeCSVRow[]) {
  ...
  for (const row of rows) {
    ...
    if (row.attribute_name && row.attribute_value) {
      const name = row.attribute_name.trim().toUpperCase();
      const value = row.attribute_value.trim();
      const key = `${name}::${value}`;
      
      if (!exercise._seen.has(key)) {
        exercise.attributes.push({
          attributeName: name,
          attributeValue: value,
        });
        exercise._seen.add(key);
      }
    }
  }
          ...
          for (const exercise of exercises) {
            try {
              ...
              for (const attr of exercise.attributes) {
                try {
                  const normalizedName = attr.attributeName.trim().toUpperCase() as ExerciseAttributeNameEnum;
                  const attributeName = await ensureAttributeNameExists(normalizedName);
